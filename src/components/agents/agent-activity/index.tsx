...
export function AgentActivity({
  ...
  // Saat working, viewport memeluk tinggi konten (bawah = tepi live).
  // Reserve penuh `maxHeight` (208px) hanya masuk akal saat konten TUMPAH;
  // dengan 1–2 item, itu meninggalkan ruang kosong besar di atas konten —
  // persis gap kosong yang terlihat antara baris working dan isinya.
  // Glide "bottom-pinned" (streamOffset negatif) tetap aktif saat konten
  // melebihi maxHeight, perilaku upstream beUI tidak berubah.
  const viewportHeight = working
    ? Math.min(Math.max(contentHeight, 0), Math.max(0, maxHeight))
    : cappedHeight;
