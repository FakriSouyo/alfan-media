...
interface SelectContentProps {
  className?: string;
  children: ReactNode;
  /** Visual scroll cue when the option list overflows its max-height.
   *  Top/bottom fades and a bottom chevron clear once the list no longer
   *  overflows in that direction. Default true. */
  scrollFade?: boolean;
}
...
const SelectContent = forwardRef<HTMLDivElement, SelectContentProps>(
  ({ className, children, scrollFade = true }, ref) => {
    ...
    // Scroll-fade cues (spec: fades that appear only when the list overflows,
    // clear at each respective edge, and never intercept pointer events).
    ...
    const updateScrollState = useCallback(() => {
      ...
      setScrollState({ canScroll, atTop: el.scrollTop <= 1, atBottom });
    }, []);
    // Re-measure on open (content, and therefore overflow, can change).
    useEffect(() => {
      if (!open) return;
      const id = requestAnimationFrame(updateScrollState);
      return () => cancelAnimationFrame(id);
    }, [open, updateScrollState]);
    const showTopFade = scrollFade && scrollState.canScroll && !scrollState.atTop;
    const showBottomFade = scrollFade && scrollState.canScroll && !scrollState.atBottom;
            ...
            <SelectContentContext.Provider value={contentCtx}>
              ...
              <SelectPrimitive.Viewport asChild>
                ...
                >
                  ...
                  {/* Scroll fades + bottom chevron — pure overlays, never in
                      the hit path (all `pointer-events-none`). */}
                  {showBottomFade && (
                    <div
                      aria-hidden
                      className="pointer-events-none absolute inset-x-2 bottom-1 z-20 flex flex-col items-center bg-gradient-to-t from-surface-2 via-surface-2/60 to-transparent pb-0.5 pt-4 text-muted-foreground"
                    >
                      <svg
                        width={12}
                        height={12}
                  ...
                  {showTopFade && (
                    <div
                      aria-hidden
                      className="pointer-events-none absolute inset-x-2 top-1 z-20 h-4 bg-gradient-to-b from-surface-2 via-surface-2/60 to-transparent"
                    />
                  )}
