  ...
  (
    ...
    useEffect(() => {
      ...
      const onKeyDown = (event: KeyboardEvent) => {
        // `event.key` can be undefined on synthetic/extension-dispatched
        // events — guard before comparing.
        if (!event.key || event.key.toLowerCase() !== shortcut.toLowerCase()) return;
      ...
      >
        ...
        >
          {variant === "floating" ? (
            ...
            <div
              data-sidebar="sidebar"
              // The rail reads one step above the page substrate so it stays a
              // distinct panel rather than merging with the canvas behind it.
              className={cn(
                "flex h-full w-full min-h-0 flex-col",
                surfaceClasses(substrate + 1, 3),
                bordered &&
                  variant === "sidebar" &&
                  (side === "left" ? "border-r border-border" : "border-l border-border")
              )}
            >
              <SurfaceProvider value={Math.min(substrate + 1, 8)}>{children}</SurfaceProvider>
            </div>
          )}
          {rail && (
          <SidebarRail
    ...
    return (
      <main
        ...
        className={cn(
          ...
          // The main region is the page substrate itself — flat page color
          // (surface-1), not a raised card. The sidebar rail carries the
          // elevation, so the canvas stays calm behind it.
          className
        )}
        {...props}
      />
